...
exports.handshake = function(options) {
  ...
  return function(req, next) {
    ...
    options.store.get(sessionID, function(err, session){
      if (err) {
        return next('Session store error: ' + err.message);
      }
      ...
      if (!session[sessionKey]) {
        return next('Passport was not initialized.');
      }

      var userKey = session[sessionKey][options.userProperty];

      if (!userKey) {
        return next('User not authorized through passport.');
      }
      ...
      options.passport.deserializeUser(userKey, function(err, user) {
        ...
        user.logged_in = true;
        options.callback(req, user, next);
      });
    });
  };
};
